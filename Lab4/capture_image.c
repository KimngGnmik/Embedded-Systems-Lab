...
void black_white(volatile short * Video_Mem_ptr){
	...
	int index = 0;				// Stores 8 bits into arrays for (320x240)/8= 9600
	int i = 0;
	int j = 0;
	int k = 0;
...
//			test = black_white[y][x];
//			printf("y: %d  x: %d Test: %d\n", y, x, test);
		}
	}
	uncompressed = 240*320;
	compressed_24 = times_compressed * 24;
	compression_ratio_24 = uncompressed/compressed_24;
	compressed_32 = times_compressed * 32;
	compression_ratio_32 = uncompressed/compressed_32;
	printf("Number of 32bits received: %d\n"
			...
			"compressed bits: %.2f\n"
			"Compression Ration: %.6f\n"
			"\n", times_compressed, uncompressed, compressed_32, compression_ratio_32, uncompressed, compressed_24, compression_ratio_24);
	writeimage(Video_Mem_ptr);			// write image
