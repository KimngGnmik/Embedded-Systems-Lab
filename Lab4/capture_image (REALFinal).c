...
#define SDRAM_IS_HERE		  0xC0000000
...
void rle_compression(volatile short * Video_Mem_ptr, volatile int * SDRAM_ptr){	
	...
	int bits_received = 0;		// Keep track of times compressed
	int uncompressed = 0;
	...
	for (y = 0; y < 9600;) {
		...
		// Outputs from RLE, Saves into SDRAM
		if(!alt_read_byte(ALT_FPGA_BRIDGE_LWH2F_OFST + RESULT_READY_PIO_BASE)){
			...
			*(SDRAM_ptr + bits_received) = alt_read_word(ALT_FPGA_BRIDGE_LWH2F_OFST + IDATA_PIO_BASE);
			alt_write_byte(ALT_FPGA_BRIDGE_LWH2F_OFST + FIFO_OUT_READ_REQ_PIO_BASE, 0);  
			bits_received++;
		}		
	}

	// Reads from SDRAM and outputs onto VGA
	for (y = 0; y < bits_received; y++){
		value = (*(SDRAM_ptr + y) & 0x00800000 ? 1 : 0);
		number = *(SDRAM_ptr + y) & 0x007FFFFF;
		for(i=0; i<number; i++){
	...
	compressed_24 = bits_received * 24;				// Total number of bits received from RLE (distorted to 24bits from 32bits)
	compression_ratio_24 = uncompressed/compressed_24;	// CR using distorted compression
	compressed_32 = bits_received * 32;				// Total number of bits received from RLE (undistorted)
	...
	printf("Number of 32bits received: %d\n"
			...
			"Compressed bits: %.0f\n"
			"Compression Ration: %.6f\n"
			, bits_received, uncompressed, compressed_32, compression_ratio_32, compressed_24, compression_ratio_24);
...
int main(void)
{
	...
	volatile int * SDRAM_ptr		= (int *) SDRAM_IS_HERE;
...
//		*(Video_In_DMA_ptr + 3)	= 0x4;		// Enable the video in preparation for next image capture
	...
	//  Converts image to black and white and displays on VGA
		...
		buffer(Video_In_DMA_ptr, KEY_ptr );	// waits for button press
		rle_compression(Video_Mem_ptr, SDRAM_ptr);		// rle compression and display
	//  Restarts video for next image capture
		buffer(Video_In_DMA_ptr, KEY_ptr );	// waits for button press
		*(Video_In_DMA_ptr + 3)	= 0x4;		// Enable the video in preparation for next image capture
	}
